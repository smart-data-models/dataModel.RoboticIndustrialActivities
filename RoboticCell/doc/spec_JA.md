...
グローバルな記述。**A Robotic cell (ロボットセル)  

## プロパティのリスト  

- `address`: 郵送先住所  - `alternateName`: このアイテムの別称  - `areaServed`: サービスや提供されるアイテムが提供される地理的なエリア  - `currentPieceNumber`: ロボットセルで実行中のジョブで行われた現在のピース数。  - `dataProvider`: 調和されたデータ・エンティティの提供者を識別する一連の文字。  - `dateCreated`: エンティティの作成タイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `dateModified`: エンティティが最後に変更された時のタイムスタンプ。これは通常、ストレージプラットフォームによって割り当てられます。  - `description`: このアイテムの説明  - `errorMessage`: エラー番号に対応するエラーメッセージを表示します。  - `errorNumber`: セルにエラー（値が0でない）が発生したかどうかを示す。  - `id`: エンティティのユニークな識別子  - `location`: アイテムへのGeojson参照。Point、LineString、Polygon、MultiPoint、MultiLineString、MultiPolygonのいずれかです。  - `name`: このアイテムの名前です。  - `owner`: オーナーのIDを参照するJSONエンコードされた文字列を含むリスト  - `refIncomingPallet`: セルに入るピースを提供するパレットのリスト。  - `refOutgoingPallet`: セルにアウトカミングピースを提供するパレットのリスト。  - `refRobot`: セルに属するロボット。  - `refVacuumPump`: セルに所属する真空ポンプ。  - `seeAlso`: アイテムに関する追加リソースを示すuriのリスト  - `source`: エンティティデータのオリジナルソースをURLで示す一連の文字。ソースプロバイダの完全修飾ドメイン名、またはソースオブジェクトのURLであることが推奨されます。  - `totalNumberOfPieces`: ロボットセルで実行されているジョブの総ピース数。  - `type`: RoboticCellである必要があります。NGSI エンティティタイプです。    
...
#### RoboticCell NGSI-v2 キーバリューの例  
RoboticCellをkey-valuesとしてJSON-LD形式で表現した例です。これは`options=keyValues`を使った場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
...
```  
#### RoboticCell NGSI-v2の正規化例  
JSON-LD形式のRoboticCellを正規化した例を示します。これは、オプションを使用しない場合のNGSI-v2との互換性があり、個々のエンティティのコンテキストデータを返します。  
...
```  
#### RoboticCell NGSI-LDのキーバリューの例  
RoboticCellをkey-valuesとしてJSON-LD形式で表現した例です。これは`options=keyValues`を使った場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
...
```  
#### RoboticCell NGSI-LDの正規化例  
RoboticCellをJSON-LD形式で正規化した例です。これはオプションを使わない場合のNGSI-LDとの互換性があり、個々のエンティティのコンテキストデータを返します。  
